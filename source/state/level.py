...
class Level(tool.State):
    ...
    def update_speed(self, newSpeed):
        self.speed = newSpeed
        menubar.plant_frozen_time_list = [t / newSpeed for t in menubar.plant_frozen_time_list]
    ...
    def setupZombies(self):
        ...
        for data in self.map_data[c.ZOMBIE_LIST]:
            self.zombie_list.append((data['time'] / self.speed, data['name'], data['map_y']))
    ...
    def play(self, mouse_pos, mouse_click):
        ...
        if self.produce_sun:
            if(self.current_time - self.sun_timer)* self.speed > c.PRODUCE_SUN_INTERVAL:
                self.sun_timer = self.current_time
    ...
    def createZombie(self, name, map_y):
        ...
        if name == c.NORMAL_ZOMBIE:
            z = zombie.NormalZombie(c.ZOMBIE_START_X, y, self.head_group)
        elif name == c.CONEHEAD_ZOMBIE:
            z = zombie.ConeHeadZombie(c.ZOMBIE_START_X, y, self.head_group)
        elif name == c.BUCKETHEAD_ZOMBIE:
            z = zombie.BucketHeadZombie(c.ZOMBIE_START_X, y, self.head_group)
        elif name == c.FLAG_ZOMBIE:
            z = zombie.FlagZombie(c.ZOMBIE_START_X, y, self.head_group)
        elif name == c.NEWSPAPER_ZOMBIE:
            z = zombie.NewspaperZombie(c.ZOMBIE_START_X, y, self.head_group)
        z.speed = self.speed
