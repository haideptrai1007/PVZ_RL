...
class Bullet(pg.sprite.Sprite):
    ...
    def update(self, game_info):
        ...
        if self.state == c.FLY:
            ...
            if(self.current_time - self.explode_timer) * self.speed > 500:
                self.kill()
...
class Plant(pg.sprite.Sprite):
    ...
    def animation(self):
        if (self.current_time - self.animate_timer) * self.speed > self.animate_interval:
            self.frame_index += 1
        ...
        self.image = self.frames[self.frame_index]
        if(self.current_time - self.hit_timer) * self.speed >= 200:
...
class SunFlower(Plant):
    ...
    def idling(self):
        ...
        elif (self.current_time - self.sun_timer) * self.speed > c.FLOWER_SUN_INTERVAL:
...
class PeaShooter(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) * self.speed > 2000:
            bullet = Bullet(self.rect.right, self.rect.y, self.rect.y, c.BULLET_PEA, c.BULLET_DAMAGE_NORMAL, False)
...
class RepeaterPea(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) * self.speed > 2000:
            bullet0 = Bullet(self.rect.right, self.rect.y, self.rect.y, c.BULLET_PEA, c.BULLET_DAMAGE_NORMAL, False)
            bullet1 = Bullet(self.rect.right + 40, self.rect.y, self.rect.y, c.BULLET_PEA, c.BULLET_DAMAGE_NORMAL, False)
            bullet0.speed = self.speed
            bullet1.speed = self.speed

            self.bullet_group.add(bullet0)
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) * self.speed > 2000:
            ...
            for i in range(3):
                ...
                bullet = Bullet(self.rect.right, self.rect.y, dest_y, c.BULLET_PEA, c.BULLET_DAMAGE_NORMAL, False)
                bullet.speed = self.speed
                self.bullet_groups[tmp_y].add(bullet)
            self.shoot_timer = self.current_time
...
class SnowPeaShooter(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) * self.speed > 2000:
            bullet = Bullet(self.rect.right, self.rect.y, self.rect.y, c.BULLET_PEA_ICE, c.BULLET_DAMAGE_NORMAL, True)
            bullet.speed = self.speed
            self.bullet_group.add(bullet)
            self.shoot_timer = self.current_time
...
class CherryBomb(Plant):
    ...
    def animation(self):
        if self.start_boom:
            ...
            elif(self.current_time - self.bomb_timer) * self.speed > 500:
                self.health = 0
        else:
            if (self.current_time - self.animate_timer) * self.speed > 100:
                self.frame_index += 1
                if self.frame_index >= self.frame_num:
                    self.setBoom()
                    return
...
class Chomper(Plant):
    ...
    def digest(self):
        ...
        elif (self.current_time - self.digest_timer) * self.speed > self.digest_interval:
            self.digest_timer = 0
...
class PuffShroom(Plant):
    ...
    def attacking(self):
        if (self.current_time - self.shoot_timer) * self.speed > 3000:
            bullet = Bullet(self.rect.right, self.rect.y + 10, self.rect.y + 10, c.BULLET_MUSHROOM, c.BULLET_DAMAGE_NORMAL, True)
            bullet.speed = self.speed
            self.bullet_group.add(bullet)
            self.shoot_timer = self.current_time
...
class PotatoMine(Plant):
    ...
    def idling(self):
        if self.is_init:
            ...
            elif (self.current_time - self.init_timer) * self.speed > 15000:
                self.changeFrames(self.idle_frames)
                self.is_init = False
...
class Squash(Plant):
    ...
    def attacking(self):
        ...
        elif (self.current_time - self.aim_timer) * self.speed > 1000:
            self.changeFrames(self.attack_frames)
