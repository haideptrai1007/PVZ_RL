...
class Zombie(pg.sprite.Sprite):
    ...
    def attacking(self):
        if self.health <= 0:
            ...
            self.helmet = False
        if (self.current_time - self.attack_timer) * self.speed > (c.ATTACK_INTERVAL * self.getTimeRatio()):
    ...
    def freezing(self):
        if self.health <= 0:
            ...
            if self.old_state == c.WALK:
                ...
                self.changeFrames(self.losthead_attack_frames)
            self.setLostHead()
        if (self.current_time - self.freeze_timer) * self.speed > c.FREEZE_TIME:
            self.setWalk()
    ...
    def animation(self):
        ...
        if (self.current_time - self.animate_timer) * self.speed > (self.animate_interval * self.getTimeRatio()):
            self.frame_index += 1
